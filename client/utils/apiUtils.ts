...
export const patchTrack = async (
  ...
  const response = await api.patch(`/tracks/patch`, {
    trackIds,
    projectId,
    accepted,
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      ...
      if (typeof reader.result === "string") {
        ...
        const binaryStringChars = Array.from(binaryArray).map((charCode) =>
...
export const getProject = async (projectId: number) => {
  const response = await api.get(`/projects/get?projectId=${projectId}`, {
    withCredentials: true,
  });
...
export const pagecountProject = async ({
  ...
  q,
  instruments,
}: { username?: string; q?: string; instruments?: string[] } = {}) => {
...
export const createProject = async (
  ...
  const response = await api.post(
    "/projects/create",
    { projectName, instrumentTypes, strictMode, description },
    { withCredentials: true }
  );
  return response.data;
};
...
export const listProjectTracks = async (username: string) => {
  const response = await api.get(`/projecttracks/list?username=${username}`, {
    withCredentials: false,
  });
...
export const createUser = async (
  ...
  const response = await api.post(
    "/users/create",
    { username, email, password, recaptchaToken, description },
    { withCredentials: false }
  );
  return response.data as string;
};
